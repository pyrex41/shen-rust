//! shen-rust REPL.
//!
//! Boots the Shen 42 kernel then routes each user line through the
...
fn run_repl() -> ExitCode {
    ...
    eprint!("shen-rust booting kernel… ");
...
fn print_banner(interp: &Interp) {
    ...
    eprintln!("\nShen {version}, ©2021–2026 Mark Tarver  (shen-rust {port}, {lang})\n");
}
...
fn klexpr_to_value(e: &KlExpr) -> Value {
    use shen_rust::value::Value as V;
    match e {
        KlExpr::Nil => V::nil(),
...
impl InternerHelper for shen_rust::symbol::Interner {
    fn clone_for_lookup(&self) -> SymIdLookup {
