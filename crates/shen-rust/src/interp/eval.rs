...
pub struct Interp {
    ...
    /// Populated during boot by `jit::install_jit` only when `SHEN_RUST_JIT`
...
impl Interp {
    ...
    /// `jit::install_jit` ran with `SHEN_RUST_JIT` set). For the differential
    ...
    fn build_closure(
        ...
        // the next. The JIT tier is a no-op unless `SHEN_RUST_JIT` installed an
    ...
    /// Returns `None` when no engine is installed (`SHEN_RUST_JIT` unset) or the
    ...
    fn do_defun(&mut self, args: &[KlExpr]) -> ShenResult<Value> {
        ...
        // With `SHEN_RUST_VM=1`, try to compile the body into bytecode,
...
/// `ClosureKind::Lambda`.
///
/// **Opt-in** (`SHEN_RUST_VM=1` or the `--served` entrypoint via
...
/// `SHEN_RUST_VM=1`, used by the `--served` entrypoint (the long-running /
