//! Ratatoskr stage-2 builder (Rust target).
//!
//! `ratatoskr-build <shaken-dir> <outdir>` reads a Ratatoskr shaken
//! directory (manifest v2: `ratatoskr.manifest.txt`, a shaken
...
/// Parsed Ratatoskr manifest v2 (`key=value` lines; unknown keys ignored).
...
fn package_name(outdir: &Path) -> String {
    ...
    if name.is_empty() || name.chars().next().is_some_and(|c| c.is_ascii_digit()) {
        name = format!("ratatoskr-{name}");
    }
    name
}

/// Locate `crates/shen-rust` relative to this builder's own manifest dir
/// (compiled in), overridable with `RATATOSKR_SHEN_RUST` for relocated
/// installs.
fn shen_rust_path() -> Result<PathBuf, String> {
    if let Ok(p) = env::var("RATATOSKR_SHEN_RUST") {
        let p = PathBuf::from(p);
        if p.join("Cargo.toml").exists() {
            return Ok(p);
        }
        return Err(format!(
            "RATATOSKR_SHEN_RUST={}: no Cargo.toml there",
            p.display()
        ));
    }
    ...
    if candidate.join("Cargo.toml").exists() {
        ...
        Err(format!(
            "could not find crates/shen-rust (looked at {}); set RATATOSKR_SHEN_RUST",
            candidate.display()
        ))
    }
}
...
fn run() -> Result<(), String> {
    ...
    if args.len() != 3 {
        return Err("Usage: ratatoskr-build <shaken-dir> <outdir>".to_string());
    }
    let shaken = PathBuf::from(&args[1]);
    let outdir = PathBuf::from(&args[2]);

    let manifest_path = shaken.join("ratatoskr.manifest.txt");
    ...
    let opts = compile_options(&format!("{} (Ratatoskr shaken kernel)", manifest.kernel));
    ...
    for user in &manifest.users {
        ...
        let opts = compile_options(&format!("{user} (Ratatoskr user code)"));
    ...
    let cargo_toml = format!(
        r#"# Auto-generated by ratatoskr-build. DO NOT EDIT.
[package]
name = "{name}"
version = "0.1.0"
edition = "2021"
...
fn generate_main(manifest: &Manifest, users: &[UserModule]) -> String {
    ...
    out.push_str(
        "//! Auto-generated by ratatoskr-build. DO NOT EDIT.\n\
         //!\n\
