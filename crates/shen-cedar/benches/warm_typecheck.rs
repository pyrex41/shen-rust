...
//!   * the engine is process-global (`SHEN_CEDAR_VM` is read once into a
...
//!   SHEN_CEDAR_VM=1 cargo run --release --bench warm_typecheck  # VM
//!
...
fn run() -> i32 {
    let vm_on = std::env::var_os("SHEN_CEDAR_VM").is_some();
    ...
    let cold_cov = shen_cedar::vm::stats::snapshot();
    ...
    let exec_cov = shen_cedar::vm::stats::snapshot();
...
fn eval_src(interp: &mut Interp, src: &str) -> Result<Value, shen_cedar::error::ShenError> {
    let expr = parse_one(src, &mut interp.symbols)
        .map_err(|e| shen_cedar::error::ShenError::new(format!("parse {src:?}: {e}")))?;
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR = <root>/crates/shen-cedar
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(|p| p.parent())
        .map(PathBuf::from)
        .expect("workspace root above crates/shen-cedar")
}
